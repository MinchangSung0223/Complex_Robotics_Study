...
function traj = ScrewVelTrajectory(Xstart, Xend, Tf, N, method)
...
for i = 1: N
    if method == 3
        ...
        sddot = CubicTimeScalingDot(Tf, timegap * (i - 1));
    else
        s = QuinticTimeScaling(Tf, timegap * (i - 1));
        sdot = QuinticTimeScalingDot(Tf, timegap * (i - 1));
        sddot = QuinticTimeScalingDot(Tf, timegap * (i - 1));
    end
    Xs = Xstart * MatrixExp6(MatrixLog6(TransInv(Xstart) * Xend) * s);
    Xdot = se3ToVec(MatrixLog6(TransInv(Xstart) * Xend)*Xs);
%     V = Adjoint(Xstart)*se3ToVec(MatrixLog6(TransInv(Xstart)*Xend));
